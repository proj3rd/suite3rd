...
export function extract(content, options = {}) {
    ...
    while (true) {
        ...
        const indexStart = matchStart.index + (includeDelimiter ? 0 : matchStart[0].length);
        const indexStop = matchStop.index + (includeDelimiter ? matchStop[0].length : 0);
